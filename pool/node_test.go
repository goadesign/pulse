...
func TestDispatchJobOneWorker(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		return numJobs(t, worker) == 1
	}, max, delay, "Worker did not receive the job within expected time")

	// Check if received payload matches dispatched payload
	assert.Equal(t, payload, worker.jobs[testName].Payload, "Received payload does not match dispatched payload")
...
func TestDispatchJobTwoWorkers(t *testing.T) {
	...
	require.Eventually(t, func() bool { return numJobs(t, worker1) == 1 }, max, delay, "Worker1 did not receive a job")
	require.Eventually(t, func() bool { return numJobs(t, worker2) == 1 }, max, delay, "Worker2 did not receive a job")

	// Verify job distribution
	assert.Contains(t, [][]byte{job1.payload, job2.payload}, worker1.jobs[job1.key].Payload, "Worker1 received unexpected payload")
	assert.Contains(t, [][]byte{job1.payload, job2.payload}, worker2.jobs[job2.key].Payload, "Worker2 received unexpected payload")
...
func TestRemoveWorkerThenShutdown(t *testing.T) {
	...
	assert.Eventually(t, func() bool { return len(handler.jobs) == 1 }, max, delay, "Job was not received by the worker")

	// Remove the worker and verify the job is removed
	assert.NoError(t, node.RemoveWorker(ctx, worker))
	assert.Eventually(t, func() bool { return len(handler.jobs) == 0 }, max, delay, "Job was not removed from the worker")
...
func TestClose(t *testing.T) {
	...
	assert.Equal(t, 0, numJobs(t, worker), "Jobs were not removed from the worker after node closure")
...
func TestTwoNodeJobDispatchAndAck(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		return len(worker2.handler.(*mockHandler).jobs) > 0
...
func TestNodeCloseAndRequeue(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		return len(worker1.handler.(*mockHandler).jobs) > 0
	...
	assert.Empty(t, worker1.handler.(*mockHandler).jobs, "Job should have been removed from worker1 after requeuing")
