...
func marshalForecast(f *genpoller.Forecast) string {
...
// unmarshalForecast unmarshals a forecast from a byte slice using binary encoding.
func unmarshalForecast(data []byte) (*genforecaster.Forecast2, error) {
	...
	periods := make([]*genforecaster.Period, periodCount)
	...
	return &genforecaster.Forecast2{
		Location: loc,
		Periods:  periods,
	}, nil
}
...
func unmarshalLocation(data []byte) (*genforecaster.Location, error) {
	buf := bytes.NewBuffer(data)
...
func unmarshalPeriod(data []byte) (*genforecaster.Period, error) {
	buf := bytes.NewBuffer(data)
